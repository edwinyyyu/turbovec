...
pub(crate) fn decode(
    ...
    // Invert the rotation: encode computed u_rot = u . R^T, so u = u_rot . R.
    let x_mat = ArrayView2::from_shape((n, dim), &x_hat).unwrap();
    let r_mat = ArrayView2::from_shape((dim, dim), rotation).unwrap();
    let u_hat = x_mat.dot(&r_mat);
    let mut reconstructed = u_hat.into_raw_vec();
...
mod tests {
    ...
    fn decode_reconstructs_directions_well() {
        ...
        let rotation = rotation_mod::make_rotation_matrix(dim);
        ...
        for i in 0..n {
            ...
            let norm_b: f64 = decoded.iter().map(|&b| (b as f64).powi(2)).sum::<f64>().sqrt();
