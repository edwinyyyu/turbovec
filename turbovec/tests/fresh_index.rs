...
fn mixture_vectors(n_clusters: usize, per_cluster: usize, seed: u64) -> Vec<f32> {
    ...
    let mut draw = |scale: f32| -> Vec<f32> {
        (0..DIM)
            .map(|_| {
                <StandardNormal as Distribution<f32>>::sample(&StandardNormal, &mut rng) * scale
            })
            .collect()
    };
...
fn recall_against(oracle_ids: &[u64], candidate_ids: &[u64], nq: usize, k: usize) -> f64 {
    let mut hits = 0usize;
    for qi in 0..nq {
        let oracle: HashSet<u64> = oracle_ids[qi * k..(qi + 1) * k].iter().copied().collect();
...
fn wal_recovers_unsaved_mutations() {
    ...
    assert_eq!(recovered.get_vector(5000).unwrap(), extra[0..DIM].to_vec(),);

    cleanup(&dir);
}

/// `remove_many` batches the WAL fsync. It must not batch away the
/// DURABILITY: everything it removed has to survive a crash before the next
/// save, exactly as a loop of `remove` calls does.
#[test]
fn remove_many_is_durable_and_matches_a_loop_of_removes() {
    let vectors = mixture_vectors(4, 100, 21);

    // Reference: one-at-a-time removes, crash, reopen.
    let ref_dir = temp_dir("rm-loop");
    let mut index = FreshIndex::new(Some(DIM), BITS).unwrap();
    index.add_with_ids(&vectors, &ids(0..400)).unwrap();
    index.save(&ref_dir).unwrap();
    for id in [3u64, 17, 200, 399, 12345] {
        index.remove(id);
    }
    let expected_len = index.len();
    drop(index);
    let ref_recovered = FreshIndex::open(&ref_dir).unwrap();

    // Same removes in one batched call, crash, reopen.
    let dir = temp_dir("rm-many");
    let mut index = FreshIndex::new(Some(DIM), BITS).unwrap();
    index.add_with_ids(&vectors, &ids(0..400)).unwrap();
    index.save(&dir).unwrap();
    // 12345 is not present, so the count must exclude it.
    assert_eq!(index.remove_many(&[3, 17, 200, 399, 12345]), 4);
    ...
    assert_eq!(
        recovered.len(),
        ref_recovered.len(),
        "batched removes did not survive the crash the way single ones do",
    );
    for id in [3u64, 17, 200, 399] {
        assert!(!recovered.contains(id), "id {id} came back after replay");
    }
    assert!(recovered.contains(4));
